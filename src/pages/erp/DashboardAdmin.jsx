...
function DashboardAdmin() {
  ...
  const Desautorizado = async (id) => {
    try {
      const response = await axios.get(`http://10.144.170.24:3001/desautorizar/${id}`, {
  ...
  const Autorizado = async (id) => {
    try {
      const response = await axios.get(`http://10.144.170.24:3001/autorizar/${id}`, {
        withCredentials: true,
      });
      if (response) {
...
useEffect(() => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const Info = await axios.get("http://10.144.170.24:3001/tableEmpresas", {
          withCredentials: true,
        });
        if (Info.status === 200) {
...
<motion.div className="logo-e-btn">
    {/* Logo da empresa */}
                {selectedItem.logo ? (
                  <img
                    src={`http://10.144.170.24:3001/uploads/Logo/${selectedItem.logo}`}
